...
I removed the '+' instruction, so repeated numbers like 1,1 mean consecutive letters, NOT the same letter from the same source word.
...
possibleInstructions = [0,1,2,3,4,'x'] # 1, 1 means get 2 consecutive letters from source word of index 1
...
def penalize_Length(word):
    return -len(word)*2 # add multiplier to make length more important
...
def constructWord(sourceWords, instructions):
    ...
    for instruction in instructions:
        if instruction == 'x':
            ...
            wordIndices[lang] += 1 # so 1, 1 means get 2 consecutive letters from source word of index 1
